...
class ItemsBloc extends Bloc<ItemsEvent, ItemsState> {
  ItemsBloc(ItemRepository itemRepository) : assert(itemRepository != null), _itemRepository = itemRepository, super(ItemsState(ItemsObtainedStatus.none));
  ...
  Stream<ItemsState> _mapEventRequestedToState(ItemsRequested event, ItemsState state) async* {
    ...
    //Unimos la dos listas, dejando de lado los items que no macheen con una section
    sections.forEach((element) {
      var itemsFiltered = items.where((x) => x.section == element.name)
          .map((e) => ItemCarta(e.name, e.description, e.price, e.image));

      section_carta.add(SectionCarta(element.name, element.image, itemsFiltered));
    });

    yield state.copyWith(status: ItemsObtainedStatus.success, sections: section_carta);
  }
